#!/usr/bin/env python3
"""
psi_validation_analysis.py

Validate L1 psi enrichment: real modification vs motif frequency artifact?

Core question: L1 psi/kb (5.17) > Control (3.60) = 1.44x.
Is this because L1 genuinely has more psi, or because L1 is AT-rich
and has more psi motif sites, leading to more MAFIA calls at constant FPR?

Analysis 1: ML probability distribution comparison
  - Extract raw ML probabilities for psi calls from MAFIA BAMs
  - Compare L1 vs Control distributions
  - If similar → signal confidence is the same, psi is "as real" in L1

Analysis 2: Per-read motif-based null model
  - Count actual psi motif sites in each read's aligned region
  - Calculate per-site modification rate = n_psi / n_motif_sites
  - Compare L1 vs Control per-site rates
  - Null model: expected psi/kb if per-site rate were identical
"""
...
def extract_psi_probs(read):
    """Extract all ML probability values for psi (chemodCode 17802) from a BAM read."""
    try:
...
def count_motifs_in_seq(seq, is_reverse):
    """Count psi motif sites in a sequence on the correct strand."""
...
def process_bam(bam_path, ref_fasta, label=""):
    ...
    Returns:
        prob_hist: np.array(256) — histogram of all psi ML probability values
        per_read: list of (n_psi_high, n_psi_all, n_motif_sites, aligned_len)
    """
    ...
    for read in bam.fetch():
        ...
        per_read.append((n_psi_high, n_psi_all, n_motifs, aligned_len))
        n += 1
...
def main():
    print("=" * 70)
    print("PSI VALIDATION: Real Modification vs Motif Frequency Artifact")
    print("=" * 70)
    ...
    for group in BASE_GROUPS:
        ...
        l1_psi_high = sum(r[0] for r in l1_reads)
        l1_motifs = sum(r[2] for r in l1_reads)
        l1_alen = sum(r[3] for r in l1_reads)

        ctrl_psi_high = sum(r[0] for r in ctrl_reads)
        ctrl_motifs = sum(r[2] for r in ctrl_reads)
        ctrl_alen = sum(r[3] for r in ctrl_reads)

        l1_rate = l1_psi_high / l1_motifs if l1_motifs > 0 else 0
        ctrl_rate = ctrl_psi_high / ctrl_motifs if ctrl_motifs > 0 else 0
        l1_psi_kb = l1_psi_high / (l1_alen / 1000) if l1_alen > 0 else 0
        ctrl_psi_kb = ctrl_psi_high / (ctrl_alen / 1000) if ctrl_alen > 0 else 0
        l1_motif_kb = l1_motifs / (l1_alen / 1000) if l1_alen > 0 else 0
        ctrl_motif_kb = ctrl_motifs / (ctrl_alen / 1000) if ctrl_alen > 0 else 0

        print(f"  L1:   {l1_n:,} reads, {l1_psi_high:,} psi, {l1_motifs:,} motif sites, "
              f"rate={l1_rate:.4f}, psi/kb={l1_psi_kb:.2f}, motif/kb={l1_motif_kb:.1f}")
        print(f"  Ctrl: {ctrl_n:,} reads, {ctrl_psi_high:,} psi, {ctrl_motifs:,} motif sites, "
              f"rate={ctrl_rate:.4f}, psi/kb={ctrl_psi_kb:.2f}, motif/kb={ctrl_motif_kb:.1f}")

        group_results.append({
            'group': group,
            'l1_reads': l1_n, 'ctrl_reads': ctrl_n,
            'l1_psi_high': l1_psi_high, 'ctrl_psi_high': ctrl_psi_high,
            ...
            'l1_psi_per_kb': l1_psi_kb, 'ctrl_psi_per_kb': ctrl_psi_kb,
    ...
    print(f"\nTotal psi modification calls: L1={l1_total_calls:,}, Control={ctrl_total_calls:,}")
    ...
    # Use histogram-based comparison (more efficient than expanding)
    ...
    # Approximate KS p-value using effective sample sizes
    ...
    l1_arr = np.array(l1_per_read_all)  # (n_psi_high, n_psi_all, n_motif_sites, aligned_len)
    ctrl_arr = np.array(ctrl_per_read_all)

    # Filter reads with ≥1 motif site (avoid division by zero)
    ...
    l1_total_psi = l1_valid[:, 0].sum()
    l1_total_motifs = l1_valid[:, 2].sum()
    l1_total_bp = l1_valid[:, 3].sum()
    ctrl_total_psi = ctrl_valid[:, 0].sum()
    ctrl_total_motifs = ctrl_valid[:, 2].sum()
    ctrl_total_bp = ctrl_valid[:, 3].sum()

    l1_agg_rate = l1_total_psi / l1_total_motifs
    ctrl_agg_rate = ctrl_total_psi / ctrl_total_motifs
    rate_ratio = l1_agg_rate / ctrl_agg_rate if ctrl_agg_rate > 0 else float('inf')

    print(f"\n--- Aggregate per-site modification rate ---")
    print(f"  L1:   {l1_total_psi:,} psi / {l1_total_motifs:,} motif sites = {l1_agg_rate:.5f} ({l1_agg_rate*100:.3f}%)")
    print(f"  Ctrl: {ctrl_total_psi:,} psi / {ctrl_total_motifs:,} motif sites = {ctrl_agg_rate:.5f} ({ctrl_agg_rate*100:.3f}%)")
    print(f"  Rate ratio (L1/Ctrl): {rate_ratio:.4f}")

    # Fisher exact test on per-site rate
    # 2x2: L1_psi, L1_nonpsi / Ctrl_psi, Ctrl_nonpsi
    l1_nonpsi = l1_total_motifs - l1_total_psi
    ctrl_nonpsi = ctrl_total_motifs - ctrl_total_psi
    or_fisher, p_fisher = stats.fisher_exact([[l1_total_psi, l1_nonpsi],
                                               [ctrl_total_psi, ctrl_nonpsi]])
    ...
    l1_psi_kb = l1_total_psi / (l1_total_bp / 1000)
    ctrl_psi_kb = ctrl_total_psi / (ctrl_total_bp / 1000)
    psi_ratio = l1_psi_kb / ctrl_psi_kb

    print(f"\n--- Decomposition of psi/kb enrichment ---")
    print(f"  L1 motif sites/kb:  {l1_motif_kb:.2f}")
    print(f"  Ctrl motif sites/kb: {ctrl_motif_kb:.2f}")
    print(f"  Motif density ratio (L1/Ctrl): {motif_ratio:.4f}")
    print(f"")
    print(f"  L1 psi/kb:  {l1_psi_kb:.3f}")
    print(f"  Ctrl psi/kb: {ctrl_psi_kb:.3f}")
    print(f"  Observed psi/kb ratio: {psi_ratio:.4f}")
    print(f"")
    print(f"  Decomposition:")
    print(f"    psi/kb ratio = motif_density_ratio × per_site_rate_ratio")
    print(f"    {psi_ratio:.4f}     = {motif_ratio:.4f}           × {rate_ratio:.4f}")
    print(f"")
    expected_psi_ratio_from_motifs = motif_ratio * 1.0  # if per-site rate identical
    excess = psi_ratio / expected_psi_ratio_from_motifs
    print(f"  If per-site rate were IDENTICAL:")
    print(f"    Expected psi/kb ratio from motif density alone: {expected_psi_ratio_from_motifs:.4f}")
    print(f"    Observed / Expected = {excess:.4f} ({(excess-1)*100:+.1f}% excess)")

    # Null model: what psi/kb would we expect if per-site rate = Control rate?
    null_l1_psi_kb = ctrl_agg_rate * l1_motif_kb
    null_ctrl_psi_kb = ctrl_agg_rate * ctrl_motif_kb
    print(f"\n--- Null model (per-site rate = Control rate for both) ---")
    print(f"  Null L1 psi/kb:  {null_l1_psi_kb:.3f}")
    print(f"  Null Ctrl psi/kb: {null_ctrl_psi_kb:.3f}")
    print(f"  Null ratio: {null_l1_psi_kb/null_ctrl_psi_kb:.4f}")
    print(f"  Observed L1 psi/kb: {l1_psi_kb:.3f}")
    print(f"  Observed excess over null: {(l1_psi_kb - null_l1_psi_kb):.3f} psi/kb "
          f"({(l1_psi_kb/null_l1_psi_kb - 1)*100:+.1f}%)")

    # Per-read distribution of per-site rates
    l1_per_read_rates = l1_valid[:, 0] / l1_valid[:, 2]  # n_psi / n_motifs
    ...
    # Bin by read length and compare rates within bins
...
1. ML Probability Distribution:
   ...
   → L1 psi calls are {'NOT' if ks_stat < 0.05 else ''} lower quality than Control calls.

2. Per-Site Modification Rate:
   L1 = {l1_agg_rate*100:.3f}% per motif site
   Ctrl = {ctrl_agg_rate*100:.3f}% per motif site
   Ratio = {rate_ratio:.4f} (Fisher p={p_fisher:.2e})

3. psi/kb Enrichment Decomposition:
   Observed psi/kb ratio (L1/Ctrl) = {psi_ratio:.4f}
   = Motif density ratio ({motif_ratio:.4f}) × Per-site rate ratio ({rate_ratio:.4f})
   Motif density explains {(motif_ratio-1)/(psi_ratio-1)*100:.0f}% of the enrichment.
   Per-site rate excess: {(rate_ratio-1)*100:+.1f}%

4. Null Model (if per-site rate were identical):
   Expected L1 psi/kb = {null_l1_psi_kb:.3f} (observed: {l1_psi_kb:.3f})
   Excess over null = {(l1_psi_kb/null_l1_psi_kb - 1)*100:+.1f}%
""")
    ...
    out_groups = os.path.join(OUT_DIR, "psi_validation_per_group.tsv")
    ...
    out_hist = os.path.join(OUT_DIR, "psi_ml_probability_histogram.tsv")
    ...
    summary = {
        ...
        'l1_total_psi_calls': int(l1_total_calls),
        'ctrl_total_psi_calls': int(ctrl_total_calls),
        'l1_ml_mean': l1_mean,
        ...
        'l1_psi_per_kb': l1_psi_kb,
        'ctrl_psi_per_kb': ctrl_psi_kb,
        'observed_psi_ratio': psi_ratio,
        'null_l1_psi_kb': null_l1_psi_kb,
        'excess_over_null_pct': (l1_psi_kb / null_l1_psi_kb - 1) * 100 if null_l1_psi_kb > 0 else 0,
    }
    df_summary = pd.DataFrame([summary])
    out_summary = os.path.join(OUT_DIR, "psi_validation_summary.tsv")
