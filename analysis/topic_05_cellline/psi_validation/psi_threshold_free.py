#!/usr/bin/env python3
"""
psi_threshold_free.py

Threshold-free psi analysis: use raw ML probabilities instead of binary cutoff.

For each motif site MAFIA evaluated, it assigns a probability (0-255).
Instead of counting sites ≥128, we use:
  - Mean probability per motif site = sum(all probs) / n_motif_sites
  - This is a continuous "modification rate" (0-255 scale, or /255 for 0-1)

Comparisons:
  1. L1 vs Control (per-site mean probability)
  2. L1 body vs flanking (within-read, per-site mean probability)
  3. Young vs Ancient L1
"""
...
def count_motifs(seq, is_reverse):
    """Count psi motif sites in sequence."""
...
def parse_all_psi_with_probs(read):
    """
    Parse ALL psi positions and their ML probabilities from MM/ML tags.
...
# =====================================================================
print("=" * 70)
print("THRESHOLD-FREE PSI ANALYSIS")
print("=" * 70)
...
# =====================================================================
print("\n" + "=" * 70)
print("PART 1: L1 vs Control — Mean probability per motif site")
print("=" * 70)

# Accumulators
l1_sum_prob = 0      # sum of all psi probabilities (0-255)
l1_n_calls = 0       # number of psi positions MAFIA reported
l1_n_motifs = 0      # number of motif sites in sequence
l1_n_reads = 0
l1_bp = 0
l1_prob_all = []     # all probabilities for distribution
...
# Also collect per-threshold counts for sweep
...
for group in BASE_GROUPS:
    ...
    for bam_path, is_l1 in [(l1_bam_path, True), (ctrl_bam_path, False)]:
        ...
        for read in bam.fetch():
            ...
            # Get ALL psi calls with probabilities
            psi_calls = parse_all_psi_with_probs(read)
            sum_prob = sum(p for _, p in psi_calls)
            n_calls = len(psi_calls)
            ...
            if is_l1:
                ...
                l1_n_reads += 1
                l1_bp += aligned_bp
                l1_prob_all.extend([p for _, p in psi_calls])
                for t in thresholds:
                    l1_by_thresh[t] += sum(1 for _, p in psi_calls if p >= t)
            else:
                ...
                ctrl_n_reads += 1
                ctrl_bp += aligned_bp
                ctrl_prob_all.extend([p for _, p in psi_calls])
                for t in thresholds:
                    ctrl_by_thresh[t] += sum(1 for _, p in psi_calls if p >= t)
...
# Continuous rate = sum(prob/255) / n_motifs
...
print(f"  {'N MAFIA psi calls (all probs)':<35s} {l1_n_calls:>12d} {ctrl_n_calls:>12d}")
print(f"  {'N motif sites':<35s} {l1_n_motifs:>12d} {ctrl_n_motifs:>12d}")
...
# MAFIA calls per motif (how many motifs get evaluated)
...
print(f"  {'MAFIA calls / motif site':<35s} {l1_calls_per_motif:>12.4f} {ctrl_calls_per_motif:>12.4f} {l1_calls_per_motif/ctrl_calls_per_motif:>7.3f}x")
...
# Re-process L1 BAMs with TE coordinate lookup
body_sum_prob = 0
body_n_calls = 0
body_n_motifs = 0
body_bp_total = 0
...
b_motif_density = body_n_motifs / (body_bp_total / 1000)
f_motif_density = flank_n_motifs / (flank_bp_total / 1000)

print(f"\n  {'Metric':<35s} {'L1 Body':>12s} {'Flanking':>12s} {'Ratio':>8s}")
print(f"  {'-'*67}")
print(f"  {'N MAFIA calls':<35s} {body_n_calls:>12d} {flank_n_calls:>12d}")
print(f"  {'N motif sites':<35s} {body_n_motifs:>12d} {flank_n_motifs:>12d}")
print(f"  {'Aligned bp':<35s} {body_bp_total:>12d} {flank_bp_total:>12d}")
print(f"  {'Motif density (/kb)':<35s} {b_motif_density:>12.2f} {f_motif_density:>12.2f} {b_motif_density/f_motif_density:>7.3f}x")
print(f"  {'MAFIA calls / motif':<35s} {b_calls_per_motif:>12.4f} {f_calls_per_motif:>12.4f} {b_calls_per_motif/f_calls_per_motif:>7.3f}x")
print(f"  {'Mean prob per call (0-255)':<35s} {b_mean_per_call:>12.1f} {f_mean_per_call:>12.1f} {b_mean_per_call/f_mean_per_call:>7.3f}x")
print(f"  {'Sum(prob)/motif (0-255 scale)':<35s} {b_mean_per_motif:>12.2f} {f_mean_per_motif:>12.2f} {b_mean_per_motif/f_mean_per_motif:>7.3f}x")
print(f"  {'Continuous rate (sum(p/255)/motif)':<35s} {b_cont_rate:>12.4f} {f_cont_rate:>12.4f} {b_cont_rate/f_cont_rate:>7.3f}x")
