#!/usr/bin/env python3
"""
m6a_young_vs_ancient.py

Per-site m6A modification rate: Young L1 vs Ancient L1 vs Control.
Uses CIGAR-aware motif counting (excluding introns).

Key question: Is there a difference in per-site m6A rate between
young L1, ancient L1, and control transcripts?
...
M6A_MOTIFS_RC = [m.translate(str.maketrans('ACGT', 'TGCA'))[::-1] for m in M6A_MOTIFS]
...
def extract_m6a_probs(read):
    """Extract m6A ML probabilities from MM/ML tags (chemodCode 21891)."""
    try:
    ...
    for entry in mm_tag.rstrip(';').split(';'):
        ...
        if '21891' in mod_spec:
            m6a_probs.extend(ml_values[ml_idx:ml_idx + n_pos])
        ml_idx += n_pos
    return m6a_probs
...
def count_motifs(seq, is_reverse):
    """Count m6A DRACH motif sites on the correct strand."""
...
def main():
    print("=" * 70)
    print("M6A VALIDATION: Young L1 vs Ancient L1 vs Control")
    ...
    data = {c: {'n_reads': 0, 'n_m6a': 0, 'n_motifs': 0, 'aligned_bp': 0,
    ...
    for group in BASE_GROUPS:
        ...
        g_young = {'n': 0, 'm6a': 0, 'motifs': 0, 'bp': 0}
        g_ancient = {'n': 0, 'm6a': 0, 'motifs': 0, 'bp': 0}
        ...
        g_ctrl = {'n': 0, 'm6a': 0, 'motifs': 0, 'bp': 0}
        ...
        for read in ctrl_bam.fetch():
            ...
            data['control']['n_m6a'] += n_m6a_high
            data['control']['n_motifs'] += n_motifs
            data['control']['aligned_bp'] += aligned_len
            for p in probs:
                if 0 <= p <= 255:
                    data['control']['prob_hist'][int(p)] += 1
            data['control']['per_read'].append((n_m6a_high, n_motifs, aligned_len))

            g_ctrl['n'] += 1
            g_ctrl['m6a'] += n_m6a_high
        ...
        for label, g in [('young', g_young), ('ancient', g_ancient), ('ctrl', g_ctrl)]:
            rate = g['m6a'] / g['motifs'] if g['motifs'] > 0 else 0
            m6a_kb = g['m6a'] / (g['bp'] / 1000) if g['bp'] > 0 else 0
            motif_kb = g['motifs'] / (g['bp'] / 1000) if g['bp'] > 0 else 0
            group_rows.append({
                'group': group, 'category': label,
                'n_reads': g['n'], 'n_m6a': g['m6a'], 'n_motifs': g['motifs'],
                'aligned_bp': g['bp'], 'per_site_rate': rate,
                'm6a_per_kb': m6a_kb, 'motif_per_kb': motif_kb,
            })
        ...
        for label, g in [('young', g_young), ('ancient', g_ancient), ('ctrl', g_ctrl)]:
            ...
            print(f"  {label:>8s}: {g['n']:>6,} reads, rate={rate:.4f}, "
                  f"m6A/kb={m6a_kb:.2f}, motif/kb={motif_kb:.1f}")
    ...
    print(f"\n{'Category':>12s}  {'Reads':>8s}  {'m6A':>8s}  {'Motifs':>10s}  "
          f"{'Rate':>8s}  {'m6A/kb':>8s}  {'Motif/kb':>9s}")

    agg = {}
    for cat in cats:
        d = data[cat]
        rate = d['n_m6a'] / d['n_motifs'] if d['n_motifs'] > 0 else 0
        m6a_kb = d['n_m6a'] / (d['aligned_bp'] / 1000) if d['aligned_bp'] > 0 else 0
        motif_kb = d['n_motifs'] / (d['aligned_bp'] / 1000) if d['aligned_bp'] > 0 else 0
        agg[cat] = {'rate': rate, 'm6a_kb': m6a_kb, 'motif_kb': motif_kb}
        print(f"  {cat:>12s}  {d['n_reads']:>8,}  {d['n_m6a']:>8,}  {d['n_motifs']:>10,}  "
              f"{rate:>8.4f}  {m6a_kb:>8.3f}  {motif_kb:>9.2f}")
    ...
    for a, b in pairs:
        ...
        a_non = da['n_motifs'] - da['n_m6a']
        b_non = db['n_motifs'] - db['n_m6a']
        or_val, p_val = stats.fisher_exact([[da['n_m6a'], a_non],
                                             [db['n_m6a'], b_non]])
        print(f"  {a} vs {b}:")
    ...
    # m6A/kb decomposition
    print(f"\n--- m6A/kb decomposition ---")
    for a, b in pairs:
        m6a_ratio = agg[a]['m6a_kb'] / agg[b]['m6a_kb'] if agg[b]['m6a_kb'] > 0 else 0
        ...
        print(f"  {a} vs {b}:")
        print(f"    m6A/kb ratio = {m6a_ratio:.4f} = motif({motif_ratio:.4f}) x rate({rate_ratio:.4f})")
    ...
    out_path = os.path.join(OUT_DIR, "m6a_young_vs_ancient_per_group.tsv")
    ...
    for cat in cats:
        ...
        summary_rows.append({
            ...
            'm6a_per_kb': agg[cat]['m6a_kb'],
            'motif_per_kb': agg[cat]['motif_kb'],
        })
    df_summary = pd.DataFrame(summary_rows)
    out_path2 = os.path.join(OUT_DIR, "m6a_young_vs_ancient_summary.tsv")
