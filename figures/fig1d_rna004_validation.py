...
from fig_style import (
    ...
    HALF_WIDTH, PANEL_HEIGHT, FS_ANNOT, FS_ANNOT_SMALL, FS_LEGEND, FS_LEGEND_SMALL,
    LW_MEDIAN, LW_BRACKET
)
setup_style()
...
# ═════════════════════════════════════════════════════════════
fig, ax_vln = plt.subplots(figsize=(HALF_WIDTH, PANEL_HEIGHT))
...
# Median lines (no value labels — readable from y-axis)
for i, (data, pos) in enumerate(zip(data_list, positions)):
    med = np.median(data)
    ax_vln.hlines(med, pos - 0.22, pos + 0.22, color='white', lw=1.6, zorder=4)
    ax_vln.hlines(med, pos - 0.22, pos + 0.22, color='black', lw=LW_MEDIAN, zorder=5)
