...
# =============================================================================
# Noncoding control pipeline: guppy → minimap2 → nanopolish → MAFIA
# Replicates the control pipeline (g_housekeeping.smk) for noncoding non-L1 reads.
#
# Usage: ./scripts/run_noncoding_pipeline.sh <GROUP> [--skip-guppy] [--skip-mafia]
#   e.g.: ./scripts/run_noncoding_pipeline.sh HeLa_1
#
...
FAST5_DIR="/scratch1/junsoopablo/IsoTENT_002_L1_noncoding/${GROUP}"
GUPPY_SAVE="/scratch1/junsoopablo/IsoTENT_002_L1_noncoding_guppy/${GROUP}"
OUT_DIR="${PROJECT}/results_group/${GROUP}/k_noncoding_ctrl"
BASECALL_DIR="${OUT_DIR}/nc_basecall"
...
echo "============================================================"
echo "Noncoding control pipeline: ${GROUP}"
...
# =============================================================================
FASTQ="${BASECALL_DIR}/${GROUP}.noncoding.fastq.gz"
SUMMARY="${BASECALL_DIR}/${GROUP}.noncoding.sequencing_summary.txt"
...
# =============================================================================
BAM="${OUT_DIR}/${GROUP}_noncoding_mapped.bam"
...
# =============================================================================
POLYA="${OUT_DIR}/${GROUP}.noncoding.nanopolish.polya.tsv.gz"
...
if [ "$SKIP_MAFIA" = true ]; then
    ...
    # (venv modifies PATH, hiding module-loaded binaries)
    module purge 2>/dev/null || true
    ...
    RODAN_FASTA="${MAFIA_OUT}/${GROUP}.noncoding.rodan.fasta"
    ...
    MAFIA_BAM="${MAFIA_OUT}/${GROUP}.noncoding.aligned.bam"
    ...
    REGIONS_BED="${MAFIA_OUT}/${GROUP}.noncoding_regions.bed"
    SITES_BED="${MAFIA_OUT}/${GROUP}.noncoding_sites.bed"
    ...
    READS_BAM="${MAFIA_OUT}/${GROUP}.noncoding.mAFiA.reads.bam"
    ...
    PILEUP_BED="${MAFIA_OUT}/${GROUP}.noncoding.mAFiA.sites.bed"
    if [ -s "${READS_BAM}" ]; then
        set +e
        pileup \
            --bam_file "${READS_BAM}" \
